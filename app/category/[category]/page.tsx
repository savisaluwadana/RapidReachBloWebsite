...
export default async function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  const { category } = await params; const label = decodeURIComponent(category); const posts = (await getPosts()).filter((post) => post.category.toLowerCase() === label.toLowerCase());

...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="article-page">
      <header className="article-hero shell article-shell">
        <div className="article-hero-top">
          <div className="eyebrow">
            ...
            <span>•</span>
            <time dateTime={post.publishedAt}>{new Intl.DateTimeFormat("en", { dateStyle: "long" }).format(new Date(post.publishedAt))}</time>
          </div>
          <span className="article-type">RapidReach Analysis / 001</span>
        </div>
        ...
        <div className="article-ledger">
          ...
          <div><span>Reading time</span><strong>{post.readingMinutes} minutes</strong></div>
      ...
      <div className="shell article-shell article-body-wrap">
        <aside className="quick-take">
          <div className="quick-take-head"><span className="section-kicker">Quick take</span><span aria-hidden="true">↓</span></div>
          <p>{post.summary}</p>
        ...
        <div className="article-body">
          ...
          <div className="article-end-mark"><span>RR</span><i /></div>
          <div className="tag-row">{post.tags.map((tag) => <Link key={tag} href={`/search?q=${encodeURIComponent(tag)}`}>#{tag}</Link>)}</div>
        </div>
      </div>

      <div className="shell article-shell">
        <Engagement slug={post.slug} title={post.title} initialLikes={post.likes}/>
      </div>

      {related.length > 0 && (
        <section className="shell related-section">
          <div className="section-heading">
            <div><span className="section-kicker">Keep reading</span><h2>More from the signal desk</h2></div>
            <Link className="quiet-link" href="/#latest">All stories ↗</Link>
          </div>
          <div className="related-grid">{related.map((item) => <ArticleCard key={item.slug} post={item} compact />)}</div>
        </section>
      )}

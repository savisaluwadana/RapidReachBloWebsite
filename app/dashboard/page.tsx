...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ submitted?: string; profile?: string }> }) {
  ...
  return (
    <section className="account-dashboard shell">
      ...
      <div className="account-stat-grid"><div><strong>{savedTools.length}</strong><span>Saved tools</span></div><div><strong>{savedPosts.length}</strong><span>Saved stories</span></div><div><strong>{preferences.followedTopics.length}</strong><span>Followed topics</span></div></div>

  ...
  return (
    <form action={savePost} className="cms-editor">
      ...
      <div className="cms-form-grid two">
        <label>Title<input name="title" required defaultValue={post?.title} placeholder="Article title" /></label>
      ...
      <div className="cms-form-grid three">
        ...
        <label>Reading minutes<input name="readingMinutes" type="number" min="1" defaultValue={post?.readingMinutes || 4} /></label>
      </div>
      <div className="cms-form-grid two">
        <label>Publish date<input name="publishedAt" type="datetime-local" defaultValue={(post?.publishedAt || new Date().toISOString()).slice(0, 16)} /></label>

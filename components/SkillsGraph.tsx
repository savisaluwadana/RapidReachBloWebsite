...
import { skills } from '@/lib/rapidreach/product-data'

export default function SkillsGraph() {
  const [selected, setSelected] = useState(skills[3].id)
  const active = skills.find((skill) => skill.id === selected) ?? skills[0]
  const next = useMemo(() => [...skills].filter((skill) => skill.score < skill.target).sort((a, b) => (b.target - b.score) - (a.target - a.score)).slice(0, 3), [])

  return (
    <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      <div className="rounded-2xl border border-white/[0.06] bg-[#090909] p-5 md:p-7">
        <div className="mb-7 flex items-center justify-between">
          <div><p className="text-sm font-medium text-white">Personal engineering map</p><p className="mt-1 text-xs text-zinc-600">Demo profile · dependency-aware competency model</p></div>
          ...
          {skills.map((skill) => {
            ...
            return (
              <button key={skill.id} onClick={() => setSelected(skill.id)} className={`w-full rounded-xl border p-4 text-left transition-colors ${selected === skill.id ? 'border-electric-cyan/30 bg-electric-cyan/[0.06]' : 'border-white/[0.05] bg-white/[0.015] hover:border-white/[0.09]'}`}>
                <div className="mb-3 flex items-center justify-between gap-4"><div><p className="text-sm font-medium text-zinc-200">{skill.name}</p><p className="mt-1 text-[11px] text-zinc-700">{skill.domain}{skill.dependency ? ` · depends on ${skills.find(s => s.id === skill.dependency)?.name}` : ''}</p></div><span className="text-sm font-semibold tabular-nums text-white">{skill.score}%</span></div>
      ...
      <aside className="space-y-5">
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
          ...
          <p className="mt-3 text-sm leading-6 text-zinc-500">RapidReach uses prerequisites, role targets, labs, and incident performance to decide what you should learn next—not just course completion.</p>

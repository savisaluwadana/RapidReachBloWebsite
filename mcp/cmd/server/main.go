...
func searchKnowledge(_ context.Context, _ *mcp.CallToolRequest, in struct {
	...
	type scored struct { entry KnowledgeEntry; score int }
	matches := []scored{}
	for _, entry := range knowledge {
		haystack := strings.ToLower(entry.Title + " " + entry.Domain + " " + entry.Summary + " " + strings.Join(entry.Tags, " "))
		score := 0
		for _, token := range strings.Fields(q) {
			if strings.Contains(haystack, token) { score++ }
		}
		if score > 0 { matches = append(matches, scored{entry: entry, score: score}) }
	}
	sort.Slice(matches, func(i, j int) bool { return matches[i].score > matches[j].score })
	if len(matches) == 0 { return textResult("No seeded RapidReach knowledge matched. Connect the production knowledge index adapter for broader retrieval.") }
...
func diagnoseIncident(_ context.Context, _ *mcp.CallToolRequest, in struct {
	...
	if len(in.Symptoms) == 0 { return textResult("Provide at least one symptom.") }
...
func recommendLearning(_ context.Context, _ *mcp.CallToolRequest, in struct {
	...
	for _, item := range in.Known { known[strings.ToLower(item)] = true }
	remaining := []string{}
	for _, item := range path { if !known[strings.ToLower(item)] { remaining = append(remaining, item) } }
...
func explainReleaseImpact(_ context.Context, _ *mcp.CallToolRequest, in struct {
	...
	if tech == "" { return textResult("technology is required") }
	...
	if strings.Contains(tech, "cilium") && strings.Contains(stack, "cilium") { impact = "High relevance: the technology is directly present. Validate deployed version, policy behavior, kernel/eBPF requirements and observable traffic before remediation or upgrade." }
...
func reviewManifest(_ context.Context, _ *mcp.CallToolRequest, in struct {
	...
	if strings.TrimSpace(m) == "" { return textResult("manifest is required") }
	findings := []string{}
	if !strings.Contains(m, "resources:") { findings = append(findings, "Add CPU/memory requests and limits appropriate to the workload.") }
	if !strings.Contains(m, "readinessprobe") { findings = append(findings, "Add a readinessProbe so traffic follows application readiness.") }
	if !strings.Contains(m, "livenessprobe") { findings = append(findings, "Consider a livenessProbe only if the application has a safe self-recovery condition.") }
	if strings.Contains(m, ":latest") { findings = append(findings, "Avoid mutable :latest image tags; pin an immutable version or digest.") }
	if !strings.Contains(m, "securitycontext") { findings = append(findings, "Review pod/container securityContext: non-root execution, privilege, capabilities and filesystem policy.") }
	if len(findings) == 0 { findings = append(findings, "No obvious seed-rule issues found. Production review should also validate RBAC, network policy, PDBs, topology, autoscaling, rollout strategy and workload-specific SLOs.") }
...
func reviewArchitecture(_ context.Context, _ *mcp.CallToolRequest, in struct {
	...
	if strings.TrimSpace(in.Description) == "" { return textResult("description is required") }
...
func compareTechnologies(_ context.Context, _ *mcp.CallToolRequest, in struct {
	...
	if len(in.Options) < 2 { return textResult("Provide at least two technology options.") }
...
func generateRunbook(_ context.Context, _ *mcp.CallToolRequest, in struct {
	...
	if strings.TrimSpace(in.Incident) == "" { return textResult("incident is required") }
...
func newServer() *mcp.Server {
	server := mcp.NewServer(&mcp.Implementation{Name: "rapidreach", Version: "0.1.0", Title: "RapidReach Engineering Intelligence"}, nil)
...
func main() {
	...
	if err := http.ListenAndServe(*httpAddr, mux); err != nil { log.Fatal(err) }
}

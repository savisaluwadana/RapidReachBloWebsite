...
function normalize(post: Record<string, unknown>): Post {
  return { slug: String(post.slug), title: String(post.title), summary: String(post.summary), content: String(post.content), category: String(post.category), author: String(post.author), publishedAt: String(post.publishedAt), updatedAt: post.updatedAt ? String(post.updatedAt) : undefined, featuredImageUrl: post.featuredImageUrl ? String(post.featuredImageUrl) : undefined, readingMinutes: Number(post.readingMinutes || 4), tags: Array.isArray(post.tags) ? post.tags.map(String) : [], keyTakeaways: Array.isArray(post.keyTakeaways) ? post.keyTakeaways.map(String) : [], relatedToolSlugs: Array.isArray(post.relatedToolSlugs) ? post.relatedToolSlugs.map(String).slice(0,12) : [], likes: Number(post.likes || 0), status: post.status === "draft" ? "draft" : "published" };
}
...
export async function searchPosts(query: string, limit = 50): Promise<Post[]> {
  const cleanQuery = query.trim().slice(0, 200);
  const safeLimit = Math.max(1, Math.min(limit, 50));
  if (!hasDatabase()) {
    const q = cleanQuery.toLowerCase();
    return starterPosts
      .filter((post) => !q || [post.title, post.summary, post.content, post.category, ...post.tags].join(" ").toLowerCase().includes(q))
      .slice(0, safeLimit);
  }
  ...
  const docs = await db.collection("posts").find(filter).sort({ publishedAt: -1 }).limit(safeLimit).toArray();
  return docs.map((doc) => normalize(doc as unknown as Record<string, unknown>));
}

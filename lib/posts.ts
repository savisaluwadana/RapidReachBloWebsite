...
function normalizeCorrections(value: unknown): PostCorrection[] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((item) => {
    if (!item || typeof item !== "object") return [];
    const record = item as Record<string, unknown>;
...
function normalize(post: Record<string, unknown>): Post {
  return {
    ...
    keyTakeaways: Array.isArray(post.keyTakeaways) ? post.keyTakeaways.map(String) : [],
    relatedToolSlugs: Array.isArray(post.relatedToolSlugs) ? post.relatedToolSlugs.map(String).slice(0, 12) : [],
    sources: normalizeSources(post.sources),
    corrections: normalizeCorrections(post.corrections),
